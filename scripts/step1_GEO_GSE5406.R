...
# GSE5406: 扩心病、缺血性心肌病 vs 非心衰对照
# 平台: GPL96 ([HG-U133A] Affymetrix Human Genome U133 Array Set)
# 平台注释文件: GPL96_annot.txt（已准备好在工作目录）
...
#提取目标GSE Group-分组 ####
# GSE5406 标题特征: "nonfailing" 为对照，"failing" 为疾病
# 若需细分为 Ischemic / Idiopathic，请自行修改
GSE_group <- pd %>%
  dplyr::select(title, geo_accession) %>%
  mutate(group = if_else(str_detect(title, regex("nonfailing", ignore_case = TRUE)) ,
                         "Normal","Disease")) %>%
...
ann <- fread('GPL96_annot.txt',header = T,sep = '\t')
