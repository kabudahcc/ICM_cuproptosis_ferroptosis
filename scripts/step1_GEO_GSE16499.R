...
# GSE16499: 15 例缺血性心衰 vs 15 例正常对照
# 平台: GPL5175 ([HuEx-1_0-st] Affymetrix Human Exon 1.0 ST Array)
# 平台注释文件: GPL5175_annot.txt（已准备好在工作目录）
...
#提取目标GSE Group-分组 ####
# GSE16499 标题特征: "Normal left ventricular function" 为对照，"Ischemic heart failure" 为疾病
GSE_group <- pd %>%
  dplyr::select(title, geo_accession) %>%
  mutate(group = if_else(str_detect(title, "Normal") ,
                         "Normal","Disease")) %>%
...
ann <- fread('GPL5175_annot.txt',header = T,sep = '\t')
